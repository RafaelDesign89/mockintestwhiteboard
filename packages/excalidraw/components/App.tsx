...
class App extends React.Component<AppProps, AppState> {
                      ...
                      >
                        <LayerUI
                          ...
                          onEditFirstSelectedElement={(data: string) => {
                            const frameElement = JSON.parse(JSON.stringify(firstSelectedElement))
                            this.setState({
                              selectedElementIds: { [firstSelectedElement.id]: true },
                            });
                            if (this.props.aiEnabled !== false && selectedElements.length === 1) {
                              this.onUpdateMagicFrameGenerate(
                                frameElement,
                                { status: "done", html: data }
                              )
                            }
                          }}
                        >
  ...
  private async onUpdateMagicFrameGenerate(
    ...
    const result = actionDeleteSelected.perform(elements, this.state, null, this)
    if (result) {
